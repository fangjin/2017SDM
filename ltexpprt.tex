...
This means that our proposed approach offers a significant advantage over current strategies that focus solely on modeling spike or burst related patterns for event detection. In the future, we will investigate the impact of $k$ when setting up the $k$-nearest-neighbors graph, and the scale level $J$ as well. We will also extend the absenteeism detection approach to some other social media dataset such as Weibo, and Facebook.
